...
export const listMemberAccounts = async (_input?: Call<{ accessToken?: string }>) =>
  callAdmin<MemberAccount[]>({ op: "list" });
...
export const createMemberAccount = async ({ data }: Call<{ username: string; password: string; name: string; memberId: string }>) => {
  ...
  return callAdmin<{ id: string }>({ op: "create", memberId: data.memberId || data.username, password: data.password, name: data.name });
};
